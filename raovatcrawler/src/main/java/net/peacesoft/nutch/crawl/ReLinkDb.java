 ...
 */
public class ReLinkDb extends Configured implements Tool, Mapper<Text, ParseData, Text, Inlinks> {

    public static final Logger LOG = LoggerFactory.getLogger(ReLinkDb.class);
    ...
    public ReLinkDb(Configuration conf) {
        setConf(conf);
    }
    ...
    public int run(String[] args) throws Exception {
        if (args.length < 2) {
            System.err.println("Usage: ReLinkDb <linkdb> (-dir <segmentsDir> | <seg1> <seg2> ...) [-force] [-noNormalize] [-noFilter]");
            ...
            System.err.println("\t-force\tforce update even if ReLinkDb appears to be locked (CAUTION advised)");
        ...
        try {
            ...
            LOG.error("ReLinkDb: " + StringUtils.stringifyException(e));
            return -1;
        }
    }
}
