...
public class RaovatIndexer extends Configured implements Tool {

    public static Logger LOG = LoggerFactory.getLogger(RaovatIndexer.class);

    public RaovatIndexer() {
        super(null);
    }

    public RaovatIndexer(Configuration conf) {
        super(conf);
    }

    public void indexSolr(String solrUrl, Path crawlDb, Path linkDb,
            List<Path> segments) throws IOException {
        boolean noCommit = !getConf().getBoolean(ReSolrConstants.COMMIT_INDEX, true);
        indexSolr(solrUrl, crawlDb, linkDb, segments, noCommit, false, null);
    }

    public void indexSolr(String solrUrl, Path inputDb, Path crawlDb, Path linkDb,
            List<Path> segments) throws IOException {
        boolean noCommit = !getConf().getBoolean(ReSolrConstants.COMMIT_INDEX, true);
        indexSolr(solrUrl, inputDb, crawlDb, linkDb, segments, noCommit, false, null);
    }
    ...
    public void indexSolr(String solrUrl, Path inputDb, Path crawlDb, Path linkDb,
            List<Path> segments, boolean noCommit, boolean deleteGone, String solrParams) throws IOException {
        indexSolr(solrUrl, inputDb, crawlDb, linkDb, segments, noCommit, deleteGone, solrParams, false, false);
    }
    ...
    public void indexSolr(String solrUrl, Path inputDb, Path crawlDb, Path linkDb,
        ...
        LOG.info("RaovatIndexer: starting at " + sdf.format(start));

        final JobConf job = new NutchJob(getConf());
        job.setJobName("index-solr " + solrUrl);

        LOG.info("RaovatIndexer: deleting gone documents: " + deleteGone);
        LOG.info("RaovatIndexer: URL filtering: " + filter);
        LOG.info("RaovatIndexer: URL normalizing: " + normalize);
        ...
        NutchIndexWriterFactory.addClassToConf(job, RaovatPoster.class);
...
//            }
            ...
            LOG.info("RaovatIndexer: finished at " + sdf.format(end) + ", elapsed: " + TimingUtil.elapsedTime(start, end));
        } catch (Exception e) {
            LOG.error("RaovatIndexer:" + e.toString());
        } finally {
...
//            }
    ...
    public int run(String[] args) throws Exception {
        if (args.length < 3) {
            System.err.println("Usage: RaovatIndexer <solr url> <crawldb> [-linkdb <linkdb>] [-params k1=v1&k2=v2...] (<segment> ... | -dir <segments>) [-noCommit] [-deleteGone] [-filter] [-normalize]");
            return -1;
        }
        ...
        try {
            ...
            LOG.error("RaovatIndexer: " + StringUtils.stringifyException(e));
            return -1;
        }
    }

    public static void main(String[] args) throws Exception {
        final int res = ToolRunner.run(NutchConfiguration.create(), new RaovatIndexer(), args);
        System.exit(res);
    }
}
