...
public class RaovatPostSignature
        implements Reducer<Text, RaovatPostSignature.SolrRecord, Text, RaovatPostSignature.SolrRecord>,
        Tool {
...
//            solrQuery.setFields(ReSolrConstants.ID_FIELD);
    ...
    public void reduce(Text key, Iterator<RaovatPostSignature.SolrRecord> values,
            OutputCollector<Text, RaovatPostSignature.SolrRecord> output, Reporter reporter)
            throws IOException {
        while (values.hasNext()) {
            ...
            try {
                ...
                LOG.info("RaovatPostSignature sign the key to signature field id " + key + " value " + solrDocs.size());
            } catch (Exception ex) {
...
//        JobClient.runJob(job);
        ...
        LOG.info("RaovatPostSignature: finished at " + sdf.format(end) + ", elapsed: " + TimingUtil.elapsedTime(start, end));
    }
